...
@baseline_registry.register_trainer(name="SS-ETP-PPO")
class RLTrainer(BaseVLNCETrainer):
    ...
    def save_checkpoint(self, iteration: int):
        ...
        torch.save(
            checkpoint,
            os.path.join(self.config.CHECKPOINT_FOLDER, f"ckpt.{iteration}.pth"),
        )
    ...
    def _set_config(self):
        ...
        if not hasattr(self.config, 'PPO'): 
            ...
            self.config.PPO.value_loss_coef = 0.5  # 值函数损失系数
            self.config.PPO.entropy_coef = 0.01  # 熵正则化系数
            self.config.PPO.max_grad_norm = 0.5  # 梯度裁剪范数
            self.config.PPO.gamma = 0.99  # 折扣因子
            self.config.PPO.gae_lambda = 0.95  # GAE lambda参数
        ...
        # 确保在修改SIMULATOR_GPU_IDS之前配置是解冻的
        # 配置已经在方法开始时解冻，所以这里不需要再次解冻
    ...
    def _initialize_policy(
        ...
        # 扩展模型以添加值函数输出
        self._extend_policy_with_value_function()
        ...
        if load_from_ckpt:
            ...
            self.scaler = ckpt_dict.get("scaler", GradScaler())
            logger.info(f"Loaded checkpoint from: {ckpt_path} at iteration {start_iter}")
        else:
            logger.info("No checkpoint found...")
        
        return start_iter

    def _extend_policy_with_value_function(self):
        """扩展策略网络，添加值函数输出"""
        # 获取原始的forward方法
        original_forward = self.policy.net.forward
        
        # 定义新的forward方法，添加值函数输出
        def new_forward(self, *args, **kwargs):
            outputs = original_forward(*args, **kwargs)
            
            # 只在导航模式下添加值函数输出
            if kwargs.get('mode') == 'navigation':
                # 获取全局特征
                global_features = outputs.get('global_features', None)
        ...
        # 替换forward方法
        import types
        self.policy.net.forward = types.MethodType(new_forward, self.policy.net)
    ...
    def _train_interval(self, interval, ml_weight, sample_ratio):
        ...
        for idx in pbar:
            # 收集轨迹数据
            rollout_data = self.rollout('train', ml_weight, sample_ratio)
            ...
            dones = dones[:traj_length]
            actions = actions[:traj_length]
            old_log_probs = old_log_probs[:traj_length]
            ...
            for step in range(traj_length):
                ...
                values[step] = values[step][:min_batch_size].detach()
                dones[step] = dones[step][:min_batch_size]
                actions[step] = actions[step][:min_batch_size].detach()
                old_log_probs[step] = old_log_probs[step][:min_batch_size].detach()
            ...
            # 初始化下一个状态的值函数估计
            next_value = torch.zeros(min_batch_size, device=self.device)
            
            # 计算GAE
            gae = torch.zeros(min_batch_size, device=self.device)
            for step in reversed(range(traj_length)):
                if step == traj_length - 1:
                    next_non_terminal = 1.0 - dones[step].float()
                    next_values = next_value
                else:
                    next_non_terminal = 1.0 - dones[step].float()
                    next_values = values[step + 1].squeeze(-1)
                
                delta = rewards[step] + gamma * next_values * next_non_terminal - values[step].squeeze(-1)
                gae = delta + gamma * gae_lambda * next_non_terminal * gae
                returns.insert(0, gae + values[step].squeeze(-1))
            ...
            old_log_probs = torch.stack(old_log_probs)  # [T, B]
            ...
            for epoch in range(ppo_epochs):
                ...
                for step in range(traj_length):
                    ...
                    # 重新计算导航逻辑
                    nav_inputs = state['nav_inputs']
                    txt_embeds = state['txt_embeds']
                    txt_masks = state['txt_masks']
                    
                    # 创建新的字典，避免修改原始数据
                    nav_inputs_copy = {k: v for k, v in nav_inputs.items()}
                    nav_inputs_copy.update({
                        'mode': 'navigation',
                        'txt_embeds': txt_embeds,
                        'txt_masks': txt_masks,
                    })
                    
                    nav_outs = self.policy.net(**nav_inputs_copy)
                    nav_logits = nav_outs['global_logits'][:min_batch_size]  # 确保维度匹配
                    nav_probs = F.softmax(nav_logits, 1)
                    
                    # 获取当前动作的log概率
                    dist = torch.distributions.Categorical(nav_probs)
                    curr_actions = actions[step * min_batch_size:(step + 1) * min_batch_size]
                    action_log_probs = dist.log_prob(curr_actions)
                    entropy = dist.entropy().mean()
                    
                    # 获取当前值函数估计
                    values_pred = nav_outs.get('state_values', torch.zeros(min_batch_size, 1).to(self.device))
                    if values_pred.size(0) > min_batch_size:
                        values_pred = values_pred[:min_batch_size]
                    
                    # 获取当前时间步的优势和回报
                    start_idx = step * min_batch_size
                    end_idx = (step + 1) * min_batch_size
                    step_advantages = advantages[start_idx:end_idx]
                    step_returns = returns[start_idx:end_idx]
                    step_old_log_probs = old_log_probs[start_idx:end_idx]
                    
                    # 计算PPO比率
                    ratio = torch.exp(action_log_probs - step_old_log_probs)
                    ...
                    # 值函数损失
                    value_loss = F.mse_loss(values_pred.squeeze(-1), step_returns)
                    
                    # 计算总损失
                    loss = policy_loss + value_loss_coef * value_loss - entropy_coef * entropy
                    loss = loss / traj_length  # 平均每个时间步的损失
                ...
                # 记录平均损失
                self.logs['policy_loss'].append(epoch_policy_loss / traj_length)
                self.logs['value_loss'].append(epoch_value_loss / traj_length)
                self.logs['entropy'].append(epoch_entropy / traj_length)
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        if mode == 'train':
            ...
            for i in range(self.envs.num_envs):
                ep_id = curr_eps[i].episode_id
                if self.config.MODEL.task_type == 'rxr':
                    goal_pos = np.array(self.gt_data[str(ep_id)]['locations'][-1]).astype(np.float)
                else:
                    # 假设R2R数据集中也有类似的目标位置信息
                    goal_pos = np.array(curr_eps[i].goals[0].position)
        ...
        for stepk in range(self.max_len):
            ...
            # 为PPO添加值函数预测
            if mode == 'train':
                # 假设policy网络已经扩展了值函数输出
                state_values = nav_outs.get('state_values', torch.zeros(nav_logits.size(0), 1).to(self.device))
            ...
            if mode == 'train':
                # 创建一个深拷贝，确保不保留计算图
                current_state = {
                    'nav_inputs': {k: v.detach() if isinstance(v, torch.Tensor) else v 
                                  for k, v in nav_inputs.items()},
                    'txt_embeds': txt_embeds.detach(),
                    'txt_masks': txt_masks.detach(),
                    'cur_pos': cur_pos,  # 这些不是张量，不需要detach
                    'cur_ori': cur_ori,
                    'stepk': stepk
                }
                ppo_states.append(current_state)
                
                # 确保值函数预测被分离
                if 'state_values' in nav_outs:
                    ppo_values.append(nav_outs['state_values'].detach())
                else:
                    ppo_values.append(torch.zeros(self.envs.num_envs, 1, device=self.device).detach())
            ...
            if feedback == 'sample':
                ...
                # 对于PPO，我们需要保存动作的log概率
                if mode == 'train':
                    log_probs = c.log_prob(a_t)
                    ppo_actions.append(a_t.detach())
                    ppo_log_probs.append(log_probs.detach())
                
                # 在训练中可以选择使用教师动作
                if mode == 'train' and sample_ratio > 0:
            ...
            # 计算PPO奖励
            if mode == 'train':
                rewards = torch.zeros(self.envs.num_envs, device=self.device)
                
                # 获取当前位置
                new_cur_pos, _ = self.get_pos_ori()
                
                for i in range(self.envs.num_envs):
                    # 计算与目标的距离
                    if dones[i]:
                        # 终点奖励：基于与真实终点的距离
                        distance_to_goal = np.linalg.norm(np.array(new_cur_pos[i]) - goal_positions[i])
                        # 距离越近，奖励越高
                        goal_reward = max(0, 3.0 - distance_to_goal)  # 如果距离小于3米，给予奖励
                        #goal_reward = QAreward + goal_reward 
                        if distance_to_goal <= 3.0:
                            goal_reward += 2.0  # 成功到达目标点的额外奖励
                        ...
                        # 可以添加进度奖励：如果距离目标更近了，给予小奖励
                        if stepk > 0:
                            prev_distance = np.linalg.norm(np.array(cur_pos[i]) - goal_positions[i])
                ...
                # 保存下一个状态
                if sum(dones) < self.envs.num_envs:
                    observations = extract_instruction_tokens(observations,self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID)
                    next_batch = batch_obs(observations, self.device)
                    next_batch = apply_obs_transforms_batch(next_batch, self.obs_transforms)
                    ppo_next_states.append(next_batch)
        ...
        if mode == 'train':
            return {
                'states': ppo_states,
                'actions': ppo_actions,
                'log_probs': ppo_log_probs,
                'rewards': ppo_rewards,
                'dones': ppo_dones,
                'values': ppo_values,
                'next_states': ppo_next_states
            }
        else:
            return loss / max(total_actions, 1)